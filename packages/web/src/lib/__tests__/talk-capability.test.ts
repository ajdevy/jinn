...
describe("asking whether voice is set up", () => {
  it("reads the capability without opening anything", async () => {
    authFetch.mockResolvedValue(json({ configured: true, provider: "openai", providers: ["openai"] }))

    const capability = await fetchTalkCapability()

    expect(capability).toEqual({ configured: true, provider: "openai", providers: ["openai"] })
  ...
  it("reads a gateway that answers nothing as not configured, rather than as ready", async () => {
    ...
    expect(await fetchTalkCapability()).toEqual({ configured: false, provider: null, providers: [] })
  })
