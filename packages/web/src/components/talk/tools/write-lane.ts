...
export function fastWrite(write: FastWrite): ToolResult {
  ...
  offerUndo(write.performed, async () => {
    await write.reverse()
    forgetCached(write.subject)
    // `entry.id` is filled in when the gateway acknowledges the entry above, so
    // an unacknowledged one links to nothing rather than to a guess.
    recordTalkAction({
      tool: UNDO_TOOL,
      subject: write.subject,
      lane: "fast",
 ...
 *  a network failure never read the same way to the model. */
