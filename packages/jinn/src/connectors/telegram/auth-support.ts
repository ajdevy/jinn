...
import { type AuthProvider, type RunCommand } from "./auth-providers.js";
...
const LEGACY_COMMANDS: Readonly<Record<string, { kind: "start"; provider: AuthProvider } | { kind: "status" } | { kind: "cancel" }>> = {
  claude: { kind: "start", provider: "claude" }, codex: { kind: "start", provider: "codex" }, status: { kind: "status" }, cancel: { kind: "cancel" },
};
const MENU_COMMANDS: Readonly<Record<string, AuthCommand>> = {
  claude: { kind: "start", provider: "claude" }, codex: { kind: "start", provider: "codex" }, status: { kind: "status" }, cancel: { kind: "cancel" },
};
...
export function isTimeout(error: unknown): boolean {
  ...
  const failure = error as { code?: unknown; timedOut?: unknown };
  return failure.code === "ETIMEDOUT" || failure.timedOut === true;
}
